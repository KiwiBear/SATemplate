if(isServer) then {
	...
	_radios = [];
	_tasks = [[-1,"Command Radio","SA_fnc_showCommandRadioMenu",true,false,false,true]] + _tasks + [[-2,"Drop Command Radio","SA_fnc_taskDropRadio",true,false,false,false,{ not(surfaceIsWater (getPos _x) || ((getPos _x) select 2) > 1 || vehicle _x != _x) }]];
	...
	["pv_cr_radios",_radios] call SA_fnc_setPublicVariable;
	["pv_cr_radio_tasks",_tasks] call SA_fnc_setPublicVariable;
	[_startPos] spawn SA_fnc_commandRadioSyncLoop;
};
...
if(!isDedicated) then {
	...
	g_my_radio_tasks = [];
	{
		_x addAction ["Take Command Radio", { 
			...
			[[_radio,_caller],"SA_fnc_takeCommandRadio",false] spawn BIS_fnc_MP; 
		}];
