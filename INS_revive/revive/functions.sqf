...
INS_REV_FNCT_add_actions = {
	...
	if (!isNull _unit && !_isTeleport) then	{
		...
		_id_action = _unit addAction [
			...
			true,									/* HideOnUse */
			"",										/* Shortcut */
			"player distance _target < 2 && !(player getVariable ""INS_REV_PVAR_is_unconscious"") && call INS_REV_FNCT_can_revive && alive _target && (_target getVariable ""INS_REV_PVAR_is_unconscious"") && isNil {_target getVariable ""INS_REV_PVAR_who_taking_care_of_injured""}"	/* Condition */
		];
