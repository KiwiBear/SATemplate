...
_player setVariable ["INS_REV_GVAR_cancel_revive", false];
...
_cancel_revive_action = _player addAction [
...
while {!isNull _player && alive _player && !isNull _injured && alive _injured && _startTime + _reviveTakeTime > time && !(_player getVariable ["INS_REV_GVAR_cancel_revive", false])} do {
	_playerMove = format ["AinvPknlMstpSnonWrflDnon_medic0", floor random 6];
	_player playMove _playerMove;
	waitUntil {animationState _player == _playerMove || (_player getVariable ["INS_REV_GVAR_cancel_revive", false]) || _startTime + _reviveTakeTime < time};
	waitUntil {animationState _player != _playerMove || (_player getVariable ["INS_REV_GVAR_cancel_revive", false]) || _startTime + _reviveTakeTime < time};
	_player playMoveNow "AinvPknlMstpSnonWrflDnon_medic";
};
...
if (INS_REV_CFG_require_medkit) then {
	...
	if !("FirstAidKit" in _itemList) then {
		...
		if !(isNull _player) then {
			if (alive _player && !(_player getVariable ["INS_REV_GVAR_cancel_revive", false])) then {
		...
		// Clear variable
		_player setVariable ["INS_REV_GVAR_cancel_revive", nil];
...
if !(isNull _injured) then {
	...
	if (!isNull _player && alive _player && alive _injured && !(_player getVariable ["INS_REV_GVAR_cancel_revive", false])) then {
		// Remove actions
		if !(PVAR_isAce) then {
